...
tryAgain:
	...
	if (fd >= 0)
	{
		uint64_t hint;										/*(beomjoon): add write hint for tpcc-like oltp workload*/
		if (strncmp("/mnt/fdp/tpcc", DataDir, 13) == 0)
		{
			hint = 1;
...
AllocateFile(const char *name, const char *mode)
{
	...
	/* Close excess kernel FDs. */
	ReleaseLruFiles();
// beomjoon - not shure to add fcntl ... fopen
TryAgain:
